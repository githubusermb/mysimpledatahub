...
# Step 1: Create collections_data_view (same schema as source table)
print(f"\n{'='*80}")
print(f"STEP 1: Creating collections_data_view")
print(f"{'='*80}")

collections_view_name = "collections_data_view"
...
# Step 2: Create series-specific report views
print(f"\n{'='*80}")
...
for seriesid_value in seriesid_values:
    ...
    try:
        # Step 1: Drop existing view if it exists (to recreate)
        try:
            glue_client.delete_table(
                DatabaseName=database_name,
                Name=view_name
...
print(f"Report views created successfully: {len(created_views)}")
...
print(f"  Athena:")
print(f"    SELECT * FROM {database_name}.collections_data_view")
print(f"  Glue Spark:")
print(f"    spark.sql('SELECT * FROM glue_catalog.{database_name}.collections_data_view')")

print(f"\nReport views (pivoted by series):")
print(f"  Athena:")
