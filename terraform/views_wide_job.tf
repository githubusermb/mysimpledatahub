# Upload Glue script for series-specific wide views to S3
resource "aws_s3_object" "glue_views_wide_script" {
  bucket = aws_s3_bucket.iceberg_data_bucket.id
  key    = "scripts/glue_create_series_wide_views.py"
  source = "../jobs/glue_create_series_wide_views.py"
  etag   = filemd5("../jobs/glue_create_series_wide_views.py")
}

# AWS Glue Job for creating series-specific wide views
# Creates pivoted views with concatenated context pattern
resource "aws_glue_job" "views_wide_job" {
  name     = "create-series-wide-views"
  role_arn = aws_iam_role.glue_service_role.arn
  
  command {
    name            = "glueetl"
    script_location = "s3://${aws_s3_bucket.iceberg_data_bucket.id}/scripts/glue_create_series_wide_views.py"
    python_version  = "3"
  }
  ...
  depends_on = [aws_s3_object.glue_views_wide_script]
}

# AWS Glue Trigger for the wide views job
